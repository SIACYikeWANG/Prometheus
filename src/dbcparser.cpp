...
void ecal_com_dp_ContructMsg(uint32 u32t_canid, DbcParserMsgTblType * pt_MsgTbl, uint16 u16_tblSize)
{
    ...
    if( u8g_ecal_com_dp_TxStatus == (uint8)OFF )
    {
        return;
    }
    for ( u16t_loop = (uint16)0 ; u16t_loop < u16_tblSize; u16t_loop++ )
    ...
    for ( u8t_loop = 0 ; u8t_loop < pt_MsgTbl[u16t_loop].u16_TblSize ; u8t_loop ++ )
    {
        ...
        case DP_SIGNALTYPE_FLOAT:
            ...
            if( ptt_sigtbl[u8t_loop].u8_SignType == DP_SIGNTYPE_SIGNED && ptt_sigtbl[u8t_loop].u8_DataLen > 1)
            {
                ...
                {
                    s64t_value = (s64t_value & 0x7FFFFFFF);//åŽ»æŽ‰æœ€é«˜ä½ç¬¦å·;
                    //s64t_value = ~s64t_value +1;//åŽ»æŽ‰æœ€é«˜ä½ç¬¦å·;
    ...
    //stt_pdu.id = u32t_canid;
    //stt_pdu.length = (uint8)u8t_msglen;
    //stt_pdu.sdu = u8t_buffer;
    //stt_pdu.swPduHandle = u8t_channel;
    if (u32t_canid > 0x7FF)
    {
        u8t_frameType = 1;
    }
